...
/**
 * تصدير عدة جداول إلى ملف Excel واحد (كل جدول في sheet منفصل)
 */
export async function exportMultipleSchedulesToExcel(
  schedules: Array<{
    name: string;
    scheduleData: any[][];
    days: string[];
  ...
  for (const schedule of schedules) {
    ...
    // العنوان
    worksheet.mergeCells(currentRow, 1, currentRow, schedule.days.length + 1);
    const titleCell = worksheet.getCell(currentRow, 1);
    titleCell.value = `${ title } - ${ schedule.name } `;
    titleCell.font = { name: 'Arial', size: settings.titleFontSize || 16, bold: true };
    titleCell.alignment = { vertical: 'middle', horizontal: 'center' };
    currentRow += 2;
    ...
    const headerRow = worksheet.getRow(currentRow);
    const dataStartRow = currentRow + 1;
    headerRow.values = ['الوقت', ...schedule.days];
    applyHeaderFormatting(worksheet, headerRow, settings);
    currentRow++;

    // بيانات الجدول
    schedule.scheduleData.forEach((rowData, index) => {
      const row = worksheet.getRow(currentRow);
      const timeSlot = schedule.timeSlots[index];
      const values = [`${ timeSlot.start } - ${ timeSlot.end } `, ...rowData.map((cell: any) => {
        if (!cell || cell.length === 0) return '';
        return cell
          .map((assignment: any) => 
    ...
    // ضبط عرض الأعمدة
    worksheet.getColumn(1).width = 20;
    for (let i = 2; i <= schedule.days.length + 1; i++) {
      worksheet.getColumn(i).width = 25;
    }
    ...
    worksheet.autoFilter = {
      from: { row: dataStartRow - 1, column: 1 },
      to: { row: currentRow - 1, column: schedule.days.length + 1 },
    };
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `${ title.replace(/\s+/g, '_') }_جميع_التخصصات_${ new Date().toISOString().split('T')[0] }.xlsx`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
